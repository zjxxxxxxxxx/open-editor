import { type Plugin } from 'rollup';
import MagicString from 'magic-string';

export interface GlslPluginOptions {
  /**
   * 控制是否生成 SourceMap，用于调试时映射压缩后的代码到原始位置。
   * @default false
   */
  sourcemap?: boolean;
}

// 匹配所有 glsl`...` 模板字符串，并捕获内部任意字符（包括换行）
const GLSL_RE = /glsl`([\s\S]*?)`/g;
// 匹配所有换行符、符号前后的多余空格，用于一步到位地压缩 GLSL 内容
const GLSL_COMPACT_RE = /[\n\r\f]+|\s+([{}[\](),;=+\-*/])|([{}[\](),;=+\-*/])\s+/g;

export default function glslPlugin(opts: GlslPluginOptions = {}): Plugin {
  return {
    name: 'rollup:glsl',

    transform(code, id) {
      // 快速路径：如果源码不包含 glsl`，则无需处理
      if (!code.includes('glsl`')) return null;

      let s!: MagicString; // MagicString 实例，用于高效修改代码

      // 使用 matchAll 迭代所有匹配项，获取匹配内容和精确位置
      for (const match of code.matchAll(GLSL_RE)) {
        // 延迟初始化 MagicString 实例，仅在第一次替换时创建
        s ??= new MagicString(code);

        const raw = match[0]; // 完整匹配的字符串，如 'glsl`...`'
        const content = match[1]; // 捕获组1的内容，即模板字符串内部
        const offset = match.index!; // 匹配项的起始索引

        // 使用一步到位的正则替换，简化 GLSL 内容的清洗过程
        const processedContent = content
          .replace(GLSL_COMPACT_RE, (_, p1, p2) => p1 || p2 || '') // 替换换行、符号前/后空格
          .trim(); // 移除最终的首尾空白

        // 用处理后的单行字符串替换原始模板字面量部分
        s.overwrite(offset, offset + raw.length, `'${processedContent}'`);
      }

      // 如果没有进行任何替换 (即 s 仍然是 undefined)，则返回 null
      if (!s) return null;

      // 返回转换后的代码和 Sourcemap
      return {
        code: s.toString(),
        map: opts.sourcemap ? s.generateMap({ source: id, file: id }) : null,
      };
    },
  };
}
