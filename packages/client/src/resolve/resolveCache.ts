 ...
 * - 典型使用场景：在DOM元素生命周期中持久化存储其关联的源码元数据
 */
export type CacheValue = {
   ...
   * - 该元数据应与其绑定的DOM元素保持相同生命周期
   * - 当元素被移除时，该元数据会随WeakMap机制自动释放
   */
  meta?: CodeSourceMeta;
};
...
/**
 * 获取指定DOM元素关联的缓存数据
 * @param el - 需要查询的DOM元素节点
 * @returns 关联的缓存数据，若不存在或已回收则返回undefined
 */
